...
function newBookForm() {
  ...
  newBookColumn.appendChild(span);
...
//validating the author and title
let validate = elements => {
  let errorMessage = document.querySelector('.error');
  for (element of elements) {
    if (element.validity.valueMissing) {
      //post an error message if the author or title is missing
      errorMessage.textContent = `Please enter the book's ${element.id}.`;
      return false;
    }
  }
  errorMessage.textContent = '';
  return true; // otherwise continue with the add new book function
};
//SUBMITTING THE RESULTS OF THE FORM
function addNewBook() {
  let title = document.getElementById('title');
  let author = document.getElementById('author');
  if (validate([title, author]) === false) {
    return;
  }
...
function changeReadStatus() {
  //toggles between the three statuses (no, yes, currently reading)
